...
注意： 项目中使用的时候， file 路径地址需要填写文件的相对路径，命令行需要在当前目录下才行。如不在当前目录下 需要使用绝对路径。
...
配置完成后，在 Cursor 中按 `Cmd+Shift+P` 搜索 `MCP`，点击 **Reload MCP Servers** 使配置生效。 因为版本或配置不同，如果配置中提示`MCP`文件无法读取，按照提示创建文件输入上面的 `JSON` 即可
