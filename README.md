...
| ------------------- | -------- | ------------------------------------------------------------- | ---------------------------------------- | -------------------------------------------- |
| `--file`            | 是       | 未传则读取配置文件 `defaultFiles`；若仍为空则报错退出         | 目标接口文件路径                         | `--file src/api/user.ts`                     |
| `--swagger`         | 是       | 未传则读取配置文件 `swaggerUrl`；若仍为空则报错退出           | Swagger 文档地址或本地 JSON 文件路径     | `--swagger https://api.example.com/doc.html` |
| `--dry-run`         | 否       | 默认 `false`                                                  | 预览模式，不修改文件                     | `--dry-run`                                  |
| `--functions`       | 否       | 不传则处理目标文件内所有待处理函数                            | 只处理指定函数（支持逗号分隔或重复传参） | `--functions getUserApi,createUserApi`       |
| `--json`            | 否       | 默认 `false`                                                  | 成功场景输出结构化 JSON                  | `--json`                                     |
| `--silent`          | 否       | 默认 `false`                                                  | 静默成功日志（失败错误仍输出）           | `--silent`                                   |
| `--mcp`             | 否       | 默认 `false`                                                  | 以 MCP Server 模式启动                   | `--mcp`                                      |
| `--endpoint-prefix` | 否       | 未传则读取配置文件 `endpointPrefix`；再没有则按空字符串处理   | 接口路径前缀（覆盖配置文件）             | `--endpoint-prefix /algo`                    |
| `--client-name`     | 否       | 未传则读取配置文件 `clientName`；再没有则默认 `requestClient` | HTTP 客户端名称（覆盖配置文件）          | `--client-name axios`                        |
...
### 本地 Swagger/OpenAPI JSON 文件格式要求

当 `--swagger` 传本地文件（如 `./openapi.json`）时，文件需满足：

- 必须是合法 JSON（建议 UTF-8 编码，扩展名通常为 `.json`）
- 顶层需是 Swagger/OpenAPI 文档对象
- 必须包含 `paths` 字段（对象）
- `openapi`（OpenAPI3）或 `swagger`（Swagger2）建议保留
- 引用模型建议放在 `components.schemas`（OpenAPI3）或 `definitions`（Swagger2）
